      ...
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg">
        ...
        <Box className="mb-8 p-4 bg-white border rounded-lg shadow-sm">
            ...
            <Box className="space-y-4">
              {comments.map((comment) => (
                <Box key={comment.id} className="border p-4 rounded-md bg-gray-50">
                  ...
                  <Rating name={`comment-rating-${comment.id}`} value={comment.rating} readOnly size="small" className="mb-1" />
          ...
          <Box className="mt-6 pt-4 border-t">
            ...
            <TextField label="Tên của bạn" fullWidth margin="normal" variant="outlined" size="small" />
            <Rating name="new-comment-rating" value={0} precision={0.5} className="mb-2" />
            <TextField label="Nội dung bình luận" multiline rows={4} fullWidth margin="normal" variant="outlined" />
            ...
            <Grid container spacing={3}>
              {relatedProducts.map((p) => (
                <Grid item xs={12} sm={6} md={3} key={p.id}>
                  <Link to={`/product/${p.id}`} className="block no-underline">
                    <Box className="border rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 bg-white overflow-hidden">
                      <img src={p.thumbnail} alt={p.name} className="w-full h-40 object-contain p-2" />
                      <Box className="p-3">
                        <Typography variant="subtitle1" className="font-semibold text-gray-900 line-clamp-2">
                          {p.name}
                        </Typography>
                        <Typography variant="body2" className="text-red-600 font-bold mt-1">
