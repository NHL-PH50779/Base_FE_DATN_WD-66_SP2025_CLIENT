...
const Shop = () => {
  ...
  useEffect(() => {
    filterProducts();
  }, [products, searchTerm, selectedCategory, selectedBrand, sortBy, priceRange, selectedRam, selectedSsd, selectedCpu]);
  ...
  const filterProducts = () => {
    ...
    if (selectedRam.length > 0) {
      filtered = filtered.filter(product => {
        ...
        return selectedRam.some(ram => productName.includes(ram.toLowerCase()));
      });
    }
    ...
    if (selectedSsd.length > 0) {
      filtered = filtered.filter(product => {
        ...
        return selectedSsd.some(ssd => 
          productName.includes(ssd.toLowerCase()) || 
          productDesc.includes(ssd.toLowerCase())
        );
      });
    }
    ...
    if (selectedCpu.length > 0) {
      filtered = filtered.filter(product => {
        const productName = product.name.toLowerCase();
        const productDesc = (product.description || '').toLowerCase();
        return selectedCpu.some(cpu => 
          productName.includes(cpu.toLowerCase()) || 
          productDesc.includes(cpu.toLowerCase())
        );
      });
    }
  ...
  const handleSsdChange = (ssd: string) => {
    setSelectedSsd(prev => 
      prev.includes(ssd) 
        ? prev.filter(s => s !== ssd)
        : [...prev, ssd]
